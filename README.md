...
5. **Eksport** — generowanie **rewersu** (AI, wspólny dla talii,
   `tla_kart/rewers.png`) oraz:
   ...
   - **Do gry/programu**: ZIP z PNG + `manifest.json`, atlas
     **Tabletop Simulator 10×7** (ostatnie pole = rewers), sprite-sheet 13×4,
     opcja „lekka ≤4096 px".
6. **Styl AI** — wybór modelu + edytowalne opisy **stylu tła** i **stylu
   postaci** (zapis w `styles.json`), podgląd składanego system-promptu,
   kopiowanie, przywracanie domyślnych. Twarde wymogi layoutu doklejane zawsze.
...
do `projekt.json`; własne style AI do `styles.json`.
...
- `tla_kart/` — szablony (nienaruszalne) + `rewers.png`, `zdjecia/` — zdjęcia
